...
Batch process recursive directory of data CSV files (generated by `data_parser.py`), into matplot graphs of time vs. acceleration (for all 3 axes).
...
## Dataset Average Graphing
#### (tools/data_avg_grapher.py)
Batch process recursive directory of data CSV files (generated by `data_parser.py`), into a single matplot graph per subdirectory. Each graph displays the average time-aligned acceration for all files in that respective directory.
