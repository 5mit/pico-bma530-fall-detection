...
int acc_init() {
   ...
   //Initialize I2C port at 100 kHz
    ...
    data = 0b00001010;  // Old data dropped, sensor time on every fram, all ram (512 B) for FIFO
...
void debug_print_fifo(uint8_t *buff, int fifo_level) {
    ...
    for (uint8_t* ptr = buff; ptr < (buff + (fifo_level)); ) {
        ...
        debug_printf("Time: %u\tHeader: %X\tX: %d\t Y: %d\t Z: %d\t\t X: %f\t Y: %f\t Z: %f\tdt:%u\n", time, header, x, y, z, (float)x * SENSITIVITY, (float)y * SENSITIVITY,(float)z * SENSITIVITY, time - prev_time);
        prev_time = time;
    }
}
